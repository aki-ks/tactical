...
public class AsmInsnReader {
    private final StackInsnVisitor<LabelNode, Integer> iv;

    public AsmInsnReader(StackInsnVisitor<LabelNode, Integer> iv, ConversionContext ctx) {
        this.iv = iv;
    }
    ...
    private void convertVarInsnNode(VarInsnNode insn) {
        ...
        switch (opcode) {
            ...
            case Opcodes.DLOAD:
            case Opcodes.ALOAD:
                iv.visitLoad(getVarLoadStoreType(opcode), insn.var);
                break;
            ...
            case Opcodes.DSTORE:
            case Opcodes.ASTORE:
                iv.visitStore(getVarLoadStoreType(opcode), insn.var);
                break;
    ...
    private void convertTypeInsnNode(TypeInsnNode insn) {
        switch (insn.getOpcode()) {
            case Opcodes.NEW:
                iv.visitNew(me.aki.tactical.conversion.asm2stack.AsmUtil.pathFromInternalName(insn.desc));
                break;

            case Opcodes.ANEWARRAY:
                RefType baseType = me.aki.tactical.conversion.asm2stack.AsmUtil.refTypeFromInternalName(insn.desc);
                iv.visitNewArray(new ArrayType(baseType, 1), 1);
                break;

            case Opcodes.CHECKCAST:
                iv.visitReferenceCast(me.aki.tactical.conversion.asm2stack.AsmUtil.refTypeFromInternalName(insn.desc));
                break;

            case Opcodes.INSTANCEOF:
                iv.visitInstanceOf(me.aki.tactical.conversion.asm2stack.AsmUtil.refTypeFromInternalName(insn.desc));
                break;
    ...
    private void convertFieldInsnNode(FieldInsnNode insn) {
        Path fieldOwner = me.aki.tactical.conversion.asm2stack.AsmUtil.pathFromInternalName(insn.owner);
        Type fieldType = me.aki.tactical.conversion.asm2stack.AsmUtil.fromDescriptor(insn.desc);
    ...
    private void convertMethodInsnNode(MethodInsnNode insn) {
        Path owner = me.aki.tactical.conversion.asm2stack.AsmUtil.pathFromInternalName(insn.owner);
        MethodDescriptor desc = me.aki.tactical.conversion.asm2stack.AsmUtil.parseMethodDescriptor(insn.desc);
    ...
    private void convertInvokeDynamicInsnNode(InvokeDynamicInsnNode insn) {
        MethodDescriptor descriptor = me.aki.tactical.conversion.asm2stack.AsmUtil.parseMethodDescriptor(insn.desc);
        ...
        if (value instanceof Integer) {
            ...
            switch (asmType.getSort()) {
                case org.objectweb.asm.Type.METHOD:
                    MethodDescriptor desc = me.aki.tactical.conversion.asm2stack.AsmUtil.parseMethodDescriptor(asmType);
                    return new MethodTypeConstant(desc.getParameterTypes(), desc.getReturnType());

                case org.objectweb.asm.Type.OBJECT:
                case org.objectweb.asm.Type.ARRAY:
                    return new ClassConstant((RefType) me.aki.tactical.conversion.asm2stack.AsmUtil.fromAsmType(asmType));
    ...
    private Handle convertMethodHandle(org.objectweb.asm.Handle handle) {
        Path owner = me.aki.tactical.conversion.asm2stack.AsmUtil.pathFromInternalName(handle.getOwner());
        ...
        switch (tag) {
            ...
            case Opcodes.H_PUTSTATIC:
                FieldRef fieldRef = new FieldRef(owner, handle.getName(), me.aki.tactical.conversion.asm2stack.AsmUtil.fromDescriptor(handle.getDesc()));
            ...
            case Opcodes.H_INVOKEINTERFACE:
                MethodDescriptor desc = me.aki.tactical.conversion.asm2stack.AsmUtil.parseMethodDescriptor(handle.getDesc());
    ...
    private void convertIIncInsnNode(IincInsnNode insn) {
        iv.visitIncrement(insn.var, insn.incr);
    }
